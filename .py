...
while True:
    ...
    t = [[1.0] * n for i in range(n)] # Создание еденичной таблицы для её последующего заполнения
    ...
    spisoksum = list() # Создание списка сумм строк

    # Заполнение списка сумм строк
    for i in range(n):
        s = 0
        for j in range(n):
            s += t[i][j]
    ...
    spisokkaf = list() # Создание списка весовых коэффициентов

    # Заполнение списка весовых коэффициентов
    for i in range(n):
        spisokkaf.append(math.floor((spisoksum[i]/sum(spisoksum)) * 100) / 100)

    spisokkaff = round(sum(spisokkaf), 2) # Округление суммы коэффициентов до 2 знаков после запятой

    # Приведение суммы к 1
    while spisokkaff > 1:
        for i in range(n):
